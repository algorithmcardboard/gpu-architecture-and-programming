...
float  calc_playground(float *, unsigned int);
...
int main(int argc, char * argv[])
{
	...
	for(i = 0; i < N; i++){
		...
		//printf("%f ",playground[index(0, i, N)]);
	}
	//printf("\n");

	for(i = 0; i < N; i++){
		playground[index(i,0,N)] = 80;
		//printf("%f ",index(i, 0, N));
	}
	//printf("\n");

	for(i = 0; i < N; i++){
		playground[index(i,N-1, N)] = 80;
		//printf("%f ",index(i, N-1, N));
	}
	//printf("\n");

	for(i = 0; i < N; i++){
		playground[index(N-1,i,N)] = 80;
		//printf("%f ",index(N-1, i, N));
	}
	//printf("\n");
	...
	//for(i = 10; i <= 30; i++){
		...
		//printf("%f ",playground[index(i, 0, N)]);
	//}
	//printf("\n");

	sum = calc_playground(playground, N);
	printf("Sum is %f\n", sum);
...
float  calc_playground(float* playground, unsigned int N){
	int i;
	float sum = 0.0;
	for(i = 0; i < N*N; i++){
...
__global__
void calculate_temperature(float* d_temp, float* d_playground, int N){
	...
	d_temp[index(row, col, N)] = (d_playground[index(row-1, col, N)] + d_playground[index(row+1, col, N)] + d_playground[index(row, col + 1, N)] + d_playground[index(row, col-1, N)])/4.0;
	//printf("%d %d :- %f  %d %d %d %d \n", row, col, d_temp[index(row, col, N)], index(row + 1, col, N), index(row - 1, col, N), index(row, col - 1, N), index(row, col + 1, N) );
